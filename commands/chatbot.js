...
const WEB_STATE = path.join(__dirname, '../data/webState.json');
...
function saveUserGroupData(data) {
    ...
    function loadWebChatbotSettings() {
        try {
            const state = JSON.parse(fs.readFileSync(WEB_STATE, 'utf8'));
            return {
                enabled: Boolean(state.chatbot?.enabled),
                personality: String(state.chatbot?.personality || 'friendly'),
                prompt: String(state.chatbot?.prompt || '')
            };
        } catch (error) {
            if (error.code !== 'ENOENT') console.error('❌ Error loading dashboard chatbot settings:', error.message);
            return { enabled: false, personality: 'friendly', prompt: '' };
        }
    }
}
...
function getRandomDelay() {
    return Math.floor(Math.random() * 450) + 250;
}
...
async function handleChatbotCommand(sock, chatId, message, match) {
    if (!match) {
        ...
        return sock.sendMessage(chatId, {
            text: `*MACHA BOT CHATBOT*\n\n*.chatbot on*\nEnable chatbot\n\n*.chatbot "personalization" on*\nEnable personalized replies\n\n*.chatbot off*\nDisable chatbot in this group`,
            quoted: message
        });
    }

    const data = loadUserGroupData();
    const normalizedMatch = match.toLowerCase().replace(/^["']personalization["']\s+/, '').trim();
...
async function handleChatbotResponse(sock, chatId, message, userMessage, senderId) {
    const data = loadUserGroupData();
    const webChatbot = loadWebChatbotSettings();
    if (!data.chatbot[chatId] && !webChatbot.enabled) return;
...
async function getAIResponse(userMessage, userContext) {
    try {
        const prompt = `
You are MACHA BOT, the fast WhatsApp assistant branded by DJ MACHA 255.
...
3. Use natural Kiswahili or English matching the user's language
...
- Natural and casual
- Match user's tone
- Use Kiswahili when the user uses Kiswahili
- Personality mode: ${userContext.settings?.personality || 'friendly'}
- Custom owner instruction: ${userContext.settings?.prompt || 'Be helpful and concise.'}
...
ABOUT YOU:
- Your name is MACHA BOT
- Mention DJ MACHA 255 when users ask who created or owns you
- You're the boss
